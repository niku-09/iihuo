...
'
' The action list (FIX_ACTIONS_FULL.csv) comes from a full
...
Const CSV_PATH    As String = "C:\Users\nikhil.mohan\Desktop\Analysis testing\audit\FIX_ACTIONS_FULL.csv"
...
Sub Main
    ...
    Close #logF
    femap.feAppMessage 0, "fix_all_v1 done. See " & OUT_DIR & "fix_all.log"
...
Function ConvertOne(eid As Long, a As Long, b As Long, c As Long) As Integer
    ...
    If wrote = 0 Then
        Call NoteFail(eid, "topology/vnode write rejected")
        Exit Function
    End If
    ...
    If okBack = 1 Then
        ...
        failCount = 0
    Else
        Call NoteFail(eid, "read-back mismatch - element did NOT change")
    End If
...
Sub NoteFail(eid As Long, reason As String)
    ...
    If failCount >= MAX_FAILS Then
        WL ""
        WL "  *** ABORTING Stage 2 after " & MAX_FAILS & " consecutive failures."
