...
'
' PutCombination signature (confirmed from a published FEMAP API
' walkthrough, not from the reference PDF):
'    PutCombination(overallScale, count, FactorsArray, LoadCaseArray)
'      overallScale - same field as "Overall" in the dialog.
'                     MUST be a real. Passing an integer makes the
'                     call silently do nothing.
'      count        - how many entries of the arrays to read
'      Factors      - Variant array of scale factors
'      LoadCases    - Variant array of load set IDs
'
' STILL UNKNOWN: which flag selects Nastran LOAD Combination
' rather than Standard. Several plausible property names are
' attempted below, each guarded. After creating, the log reports
' what actually came back so the right one can be identified.
'--------------------------------------------------------------
Function CreateCombo(newID As Long, idx As Integer) As Integer
    ...
    oLS.title = cTitle(idx)
    Err = 0

    ' --- try to flag this as a Nastran LOAD combination ---
    ' harmless if the property does not exist
    ...
    ' --- the combination itself. overall scale must be a real. ---
    oLS.PutCombination 1#, cnt, vFac, vIDs                'signature from published walkthrough

    oLS.Put newID                                         'VERIFIED pattern
    ...
    ' report whatever the build exposes, so the Standard vs
    ' Nastran question can be settled from the log
    gotBack = 0
    On Error Resume Next
    WL "    set " & newID & " created, title=" & oLS.title
    If Err = 0 Then
        gotBack = 1
    End If
    Err = 0
    ...
    If gotBack = 0 Then
        Call NoteFail(newID, "created but could not be read back")
        Exit Function
    End If
